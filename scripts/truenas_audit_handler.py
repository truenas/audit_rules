...
class TNAuditHandler(logging.handlers.SysLogHandler):
    ...
    and friends
    """
    def __init__(self, address=None, pending_queue=None):
        self.pending_queue = pending_queue or deque()
...
class AuditdHandler:
    ...
    def terminate(self):
        if not self.syslog_queue_listener:
            return

        self.syslog_queue_listener.stop()
        if not self.syslog_handler.drain_send_queue():
            raise RuntimeError('Failed to drain send queue')
    ...
    async def run(self):
        ...
        await self.loop.add_signal_handler(signal.SIGTERM, self.terminate)
        await self.loop.add_signal_handler(signal.SIGINT, self.terminate)
        while True:
...
def main():
    ...
    handler = AuditdHandler(args.audit_socket, args.syslog_socket, loop)
